...
class MFNN(torch.nn.Module):
    ...
    def __init__(self, input_dims, output_dims, nonlinearity='tanh',bias=True):
        super().__init__()
        ...
        torch.nn.init.kaiming_uniform_(self.U, mode = 'fan_in', nonlinearity = nonlinearity)
        
        self.V = torch.nn.Parameter( torch.empty([input_dims[1], output_dims[1]])  ) #Shape = [c, c_prime]
        torch.nn.init.kaiming_uniform_(self.V, mode = 'fan_in', nonlinearity = nonlinearity)
        
        if self.bias:
            self.B = torch.nn.Parameter( torch.empty(output_dims)  ) #Shape = [r_prime, c_prime]
            torch.nn.init.kaiming_uniform_(self.B, mode = 'fan_in', nonlinearity = nonlinearity)
...
class MFNN2(torch.nn.Module):
    ...
    def __init__(self, P_input_dims, Q_input_dims, output_dims, nonlinearity = 'tanh'):
        super().__init__()
        ...
        torch.nn.init.kaiming_uniform_(self.Up, mode = 'fan_in', nonlinearity = nonlinearity)
        
        self.Vp = torch.nn.Parameter( torch.empty([P_input_dims[1], output_dims[1]])  ) #Shape = [c1, c_prime]
        torch.nn.init.kaiming_uniform_(self.Vp, mode = 'fan_in', nonlinearity = nonlinearity)

        self.Uq = torch.nn.Parameter( torch.empty([output_dims[0], Q_input_dims[0]])  ) #Shape = [r_prime, r2]
        torch.nn.init.kaiming_uniform_(self.Uq, mode = 'fan_in', nonlinearity = nonlinearity)
        
        self.Vq = torch.nn.Parameter( torch.empty([Q_input_dims[1], output_dims[1]])  ) #Shape = [c2, c_prime]
        torch.nn.init.kaiming_uniform_(self.Vq, mode = 'fan_in', nonlinearity = nonlinearity)
...
class MFNN3(torch.nn.Module):
    ...
    def __init__(self, P_input_dims, Q_input_dims, R_input_dims, output_dims, nonlinearity = 'tanh'):
        super().__init__()
        ...
        torch.nn.init.kaiming_uniform_(self.Ur, mode = 'fan_in', nonlinearity = nonlinearity)
        
        self.Vr = torch.nn.Parameter( torch.empty([R_input_dims[1], output_dims[1]])  ) #Shape = [c1, c_prime]
        torch.nn.init.kaiming_uniform_(self.Vr, mode = 'fan_in', nonlinearity = nonlinearity)
...
class MRNNCell(torch.nn.Module):
    ...
    def __init__(self, input_dims, state_dims, mem_slot_dims):
        ...
        self.Connections = MFNN3(P_input_dims=state_dims, Q_input_dims=input_dims, R_input_dims=mem_slot_dims, output_dims=state_dims, nonlinearity='leaky_relu')
        self.relu = torch.nn.LeakyReLU(negative_slope=1e-2)
