...
class MFNN4(torch.nn.Module):
    
    def __init__(self, P_input_dims, Q_input_dims, R_input_dims, S_input_dims, output_dims):
        super().__init__()
        ...
        self.Us = torch.nn.Parameter( torch.empty([output_dims[0], S_input_dims[0]])  ) #Shape = [r_prime, r1]
        torch.nn.init.kaiming_uniform_(self.Us)
        
        self.Vs = torch.nn.Parameter( torch.empty([S_input_dims[1], output_dims[1]])  ) #Shape = [c1, c_prime]
        torch.nn.init.kaiming_uniform_(self.Vs)
    ...
    def forward(self, P, Q, R, S):
        ...
        Y = torch.matmul( torch.matmul(self.Up, P), self.Vp ) + torch.matmul( torch.matmul(self.Uq, Q), self.Vq ) + torch.matmul( torch.matmul(self.Ur, R), self.Vr ) + torch.matmul( torch.matmul(self.Us, S), self.Vs ) + self.B
...
class MRNNCell(torch.nn.Module):
    ...
    def __init__(self, input_dims, state_dims, mem_slot_dims):
        ...
        self.Connections = MFNN4(P_input_dims=state_dims, Q_input_dims=input_dims, R_input_dims=mem_slot_dims, S_input_dims=state_dims, output_dims=state_dims)
        self.relu = torch.nn.ReLU()      #^Prev_time_hiiden                                                           #^Prev_layer_hidden
        
        
        
    def forward(self, hidden_prev, X_t, read_prev, hidden_layer_prev):
        """
        X_t : Tensor of shape [batch_size, *input_dims].
        """
        #assert list(X_t.shape)[1:] == self.input_dims
        hidden_t = torch.tanh( self.Connections(P=hidden_prev, Q=X_t, R=read_prev, S=hidden_layer_prev) )
                                                                                   #^Since we assume that each layer will have identical hidden dimensions.
        return hidden_t
